package com.emaua.testcases;

import org.testng.Assert;
import org.testng.annotations.AfterMethod;
import org.testng.annotations.BeforeMethod;
import org.testng.annotations.Test;

import com.emaua.base.TestBase;
import com.emaua.pages.HomePage;
import com.emaua.pages.SyllabusPage;

public class SyllabusPageTest extends TestBase{
	
	HomePage homePage;
	SyllabusPage syllabusPage;
	
	
	public SyllabusPageTest() {
		super();
	}

	@BeforeMethod
	public void setUp() {
		initialization();
		homePage = new HomePage();
		syllabusPage = new SyllabusPage();
	}
	
	@Test(enabled=false)
	public void loginPageTitleTest() {
		String title = syllabusPage.validateSyllabusPageTitle();
		Assert.assertEquals(title, "About :: Emaua");
	}
	
	@Test(enabled=false)
	public void emauaLogoTest() {
		boolean flag = syllabusPage.validateEmauaLogo();
		Assert.assertTrue(flag);
	}
	
	@AfterMethod
	public void tearDown() {
		driver.quit();
	}

}
